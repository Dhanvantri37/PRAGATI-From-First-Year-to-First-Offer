      ...
      {sorted.map(d=>(
        <div key={d.dept}>
          <div style={{ display:'flex', justifyContent:'space-between', marginBottom:3 }}>
            <span style={{ fontSize:'.75rem', fontWeight:700, color:txt }}>{d.dept}</span>
            <span style={{ fontSize:'.7rem', color:sub }}>{d.count} students · avg {d.avgScore}pts</span>
          </div>
          <div style={{ height:8, background:secBg, borderRadius:999, overflow:'hidden' }}>
      ...
      {buckets.map((b,i)=>{
        ...
        return (
          <div key={i} style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', gap:3 }}>
            ...
            <span style={{ fontSize:'.52rem', color:muted }}>{b.lo}</span>
          </div>
        );
      })}
    </div>
...
export default function FacultyLeaderboardPage() {
  ...
  // Auto-refresh every 60 seconds for live leaderboard
  useEffect(() => {
  ...
  return (
    <div style={{ fontFamily:"'Nunito',sans-serif", background:pageBg, minHeight:'100%' }}>
      {/* Hero */}
      ...
      <div style={{ display:'flex', gap:8, marginBottom:16, alignItems:'center', flexWrap:'wrap' }}>
        {[['table','📋 Rankings'],['analytics','📊 Analytics']].map(([v,l])=>(
          <button key={v} onClick={()=>setView(v)}
            style={{ padding:'8px 18px', borderRadius:9, border:`1.5px solid ${view===v?'#531697':inpBrd}`, background:view===v?GRAD:inpBg, color:view===v?'#fff':sub, fontWeight:800, cursor:'pointer', fontFamily:"'Nunito',sans-serif", fontSize:'.82rem' }}>
            {l}
          </button>
        ))}
        <div style={{ flex:1 }}/>
        <button onClick={load} disabled={loading}
          style={{ padding:'8px 14px', borderRadius:9, border:`1px solid ${inpBrd}`, background:inpBg, color:'#531697', fontWeight:800, cursor:'pointer', fontFamily:"'Nunito',sans-serif", fontSize:'.78rem' }}>
      ...
      {view==='analytics' && !loading && (
        <div style={{ display:'grid', gap:16 }}>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14 }}>
            <div style={{ background:cardBg, borderRadius:14, border:`1px solid ${cardBrd}`, padding:'18px 20px' }}>
              <div style={{ fontSize:'.68rem', fontWeight:800, color:muted, letterSpacing:'.06em', marginBottom:14 }}>SCORE DISTRIBUTION</div>
              ...
              <div style={{ display:'flex', gap:10, marginTop:12, flexWrap:'wrap' }}>
                {[['0–39','At Risk','#ef4444'],['40–69','Average','#f59e0b'],['70+','Good','#47d372']].map(([r,l,c])=>(
                  <div key={r} style={{ display:'flex', alignItems:'center', gap:5 }}>
                    ...
                    <span style={{ fontSize:'.62rem', color:sub }}>{r} {l}</span>
                  </div>
                ))}
              </div>
            </div>
            <div style={{ background:cardBg, borderRadius:14, border:`1px solid ${cardBrd}`, padding:'18px 20px' }}>
              <div style={{ fontSize:'.68rem', fontWeight:800, color:muted, letterSpacing:'.06em', marginBottom:14 }}>DEPT AVG SCORE</div>
              <DeptChart students={students} dm={dm}/>
            </div>
          </div>
          <div style={{ background:cardBg, borderRadius:14, border:`1px solid ${cardBrd}`, padding:'18px 20px' }}>
            <div style={{ fontSize:'.68rem', fontWeight:800, color:muted, letterSpacing:'.06em', marginBottom:14 }}>SKILL LEVEL DISTRIBUTION</div>
              ...
              {['Beginner','Intermediate','Expert'].map(level=>{
                ...
                return (
                  <div key={level} style={{ textAlign:'center', padding:'14px', borderRadius:12, border:`2px solid ${col}22`, background:`${col}08` }}>
                    ...
                    <div style={{ fontSize:'.78rem', fontWeight:700, color:txt, margin:'4px 0' }}>{level}</div>
                    <div style={{ fontSize:'.65rem', color:muted }}>{pct}% of students</div>
                    <div style={{ height:5, background:secBg, borderRadius:999, marginTop:8, overflow:'hidden' }}>
          ...
          <div style={{ background:cardBg, borderRadius:14, border:`1px solid ${cardBrd}`, padding:'18px 20px' }}>
            <div style={{ fontSize:'.68rem', fontWeight:800, color:muted, letterSpacing:'.06em', marginBottom:14 }}>TOP SCORER PER DEPARTMENT</div>
        ...
        <>
          <div style={{ display:'flex', gap:8, flexWrap:'wrap', marginBottom:14 }}>
            <input value={search} onChange={e=>setSearch(e.target.value)}
              ...
              style={{ flex:1, minWidth:180, padding:'8px 12px', borderRadius:9, border:`1.5px solid ${inpBrd}`, fontFamily:"'Nunito',sans-serif", fontSize:'.82rem', outline:'none', background:inpBg, color:txt }}/>
            <select value={filterBranch} onChange={e=>setFB(e.target.value)}
              style={{ padding:'8px 10px', borderRadius:9, border:`1.5px solid ${inpBrd}`, fontFamily:"'Nunito',sans-serif", fontSize:'.78rem', fontWeight:700, background:inpBg, color:txt, cursor:'pointer' }}>
            ...
            <div style={{ display:'flex', border:'1.5px solid #d0d7e8', borderRadius:9, overflow:'hidden' }}>
              {[['score','🏅 Score'],['streak','🔥 Streak'],['solved','💻 Solved'],['apt','🎯 Apt']].map(([k,l])=>(
                <button key={k} onClick={()=>setSortBy(k)}
                  style={{ padding:'8px 11px', border:'none', background:sortBy===k?GRAD:inpBg, color:sortBy===k?'#fff':sub, fontWeight:700, cursor:'pointer', fontFamily:"'Nunito',sans-serif", fontSize:'.72rem' }}>
                  {l}
                </button>
              ))}
            </div>
          ...
          <div style={{ fontSize:'.7rem', color:muted, marginBottom:14, fontWeight:600 }}>
          ...
          {loading ? (
            <div style={{ textAlign:'center', padding:40 }}>
              ...
              <div style={{ color:muted, fontSize:'.82rem' }}>Computing dynamic rankings…</div>
            </div>
          ) : (
            <>
              {/* Podium top-3 */}
                  ...
                  {[top3[1],top3[0],top3[2]].map((s,pi)=>{
                    ...
                    return (
                      <div key={s._id} style={{ background:'#fff', borderRadius:16, padding:'16px 12px', textAlign:'center', border:`2px solid ${pc}40`, boxShadow:`0 4px 20px ${pc}20` }}>
                        ...
                        <div style={{ fontSize:'.58rem', color:muted, marginBottom:6 }}>points</div>
                        <div style={{ display:'flex', justifyContent:'center', gap:8, fontSize:'.62rem', color:sub }}>
              ...
              {/* Table */}
              <div style={{ background:cardBg, borderRadius:14, border:`1px solid ${cardBrd}`, overflow:'hidden' }}>
                <div style={{ display:'grid', gridTemplateColumns:'50px 1fr 90px 72px 72px 72px 80px', padding:'10px 16px', background:secBg, borderBottom:`1px solid ${cardBrd}`, fontSize:'.62rem', fontWeight:800, color:muted, letterSpacing:'.06em' }}>
                ...
                {filtered.map(s=>{
                  ...
                  return (
                    <div key={s._id}
                      style={{ display:'grid', gridTemplateColumns:'50px 1fr 90px 72px 72px 72px 80px', padding:'11px 16px', borderBottom:`1px solid ${cardBrd}`, transition:'background .12s', alignItems:'center' }}
                      ...
                      <div style={{ display:'flex', alignItems:'center', gap:9, minWidth:0 }}>
                        ...
                        <div style={{ minWidth:0 }}>
                          ...
                          <div style={{ fontSize:'.62rem', color:muted }}>{s.department} · Y{s.year}{s.rollNumber?` · ${s.rollNumber}`:''}</div>
                        </div>
                      </div>
