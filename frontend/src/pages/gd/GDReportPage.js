...
function ScoreRing({ score, label, color, size = 90 }) {
  ...
  return (
    <div style={{ textAlign: 'center' }}>
      ...
      <div style={{ marginTop: -size/2-4, fontFamily: "'Syne',sans-serif", fontWeight: 800, fontSize: size > 80 ? '1.3rem' : '.9rem', color: 'var(--text)' }}>{score}</div>
      <div style={{ marginTop: size/2-8, fontSize: '.68rem', fontWeight: 700, color: 'var(--text-3)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>{label}</div>
    </div>
  );
}
...
export default function GDReportPage() {
  ...
  if (!data?.participant) return (
    <div style={{ textAlign: 'center', padding: 60 }}>
      ...
      <div style={{ color: 'var(--text-3)' }}>Report not available yet. Please wait for evaluation to complete.</div>
  ...
  }[scores.placementReadiness] || 'var(--text-3)';
  ...
  return (
    <div style={{ fontFamily: "'Nunito',sans-serif", maxWidth: 960, margin: '0 auto', padding: '0 8px 40px' }}>
      ...
      <div className="card" style={{ padding: '28px 32px', marginBottom: 16, textAlign: 'center' }}>
        ...
        <div style={{ fontFamily: "'Syne',sans-serif", fontWeight: 800, fontSize: '1.1rem', color: 'var(--text)', marginBottom: 10 }}>{overallLabel}</div>
        ...
        <div className="card" style={{ padding: '20px 16px' }}>
          ...
          <ResponsiveContainer width="100%" height={220}>
            <RadarChart data={radarData}>
              ...
              <PolarAngleAxis dataKey="subject" tick={{ fontSize: 10, fill: 'var(--text-3)', fontFamily: "'Nunito',sans-serif" }} />
        ...
        <div className="card" style={{ padding: '20px 24px', marginBottom: 14 }}>
          ...
          {scores.summary && (
            <div style={{ marginBottom: 12, padding: '12px 16px', borderRadius: 10, background: 'rgba(83,22,151,0.06)', border: '1px solid rgba(83,22,151,0.12)' }}>
              ...
              <div style={{ fontSize: '.85rem', color: 'var(--text-2)', lineHeight: 1.7 }}>{scores.summary}</div>
            </div>
          )}
          {scores.detailedFeedback && (
            <div style={{ padding: '12px 16px', borderRadius: 10, background: 'rgba(19,161,165,0.05)', border: '1px solid rgba(19,161,165,0.12)' }}>
              ...
              <div style={{ fontSize: '.85rem', color: 'var(--text-2)', lineHeight: 1.7 }}>{scores.detailedFeedback}</div>
            </div>
          )}
        </div>
      )}
          ...
          <div className="card" style={{ padding: '18px 20px' }}>
            ...
            {scores.strengths.map((s, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 8, padding: '7px 0', borderBottom: '1px solid #f0f3fa' }}>
                ...
                <span style={{ fontSize: '.83rem', color: 'var(--text-2)', lineHeight: 1.5 }}>{s}</span>
              </div>
            ))}
          </div>
        )}
          ...
          <div className="card" style={{ padding: '18px 20px' }}>
            ...
            {scores.improvements.map((imp, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 8, padding: '7px 0', borderBottom: '1px solid #f0f3fa' }}>
                ...
                <span style={{ fontSize: '.83rem', color: 'var(--text-2)', lineHeight: 1.5 }}>{imp}</span>
              </div>
            ))}
          </div>
        )}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10 }}>
            {[
              ...
              <div key={label} style={{ textAlign: 'center', padding: '12px 8px', borderRadius: 10, background: '#f8faff' }}>
                ...
                <div style={{ fontFamily: "'Syne',sans-serif", fontWeight: 800, fontSize: '1.1rem', color: 'var(--text)' }}>{val}</div>
      ...
      <div style={{ display: 'flex', gap: 12, justifyContent: 'center' }}>
        ...
        <button onClick={() => window.print()} style={{ padding: '12px 28px', borderRadius: 12, border: '1.5px solid #d0d7e8', background: 'transparent', color: 'var(--text-2)', fontWeight: 700, cursor: 'pointer', fontFamily: "'Nunito',sans-serif" }}>
          🖨️ Print Report
        </button>
      </div>
    </div>
