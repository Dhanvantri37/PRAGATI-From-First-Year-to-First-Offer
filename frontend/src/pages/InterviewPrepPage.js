/**
 * InterviewPrepPage — v3.0 COMPLETE REWRITE
 * 
 * This is the page that actually works — accessed via /dashboard/interview-prep
 * through InterviewPrepHub → PracticeRoundPage is NOT used for AI interview.
 * This page IS the AI interview feature.
 *
 * Features:
 *  - Human-like AI avatar with animated face (blink, lip-sync, gaze, breathing)
 *  - Continuous voice recognition — click once, speak freely, auto-sends on 2.5s silence  
 *  - Dynamic AI questions — every question generated from YOUR previous answer
 *  - Tech-stack detection (React/Java/Python/DB/Cloud/ML) → targeted drilldowns
 *  - Real-time answer scoring (words, WPM, fillers, STAR, examples)
 *  - AI speaks questions aloud via Web Speech TTS
 *  - Session summary with per-question bar chart
 *  - Full Prep guide + Deep Dive preserved
 *  - Questions Bank preserved
 */
...
// Click once → listens until stopped → auto-sends answer after 2.5s silence
// ─────────────────────────────────────────────────────────────────────────────
function useContinuousSTT({ lang = 'en-IN', silenceMs = 2500, onPartial, onFinal, onError } = {}) {
  ...
  const startSession = useCallback(() => {
    ...
    r.lang = lang; r.maxAlternatives = 1;
    recRef.current = r;
    ...
    r.onresult = e => {
      ...
      if (newFinal) { finalRef.current += newFinal; armSilence(); }
    ...
    r.onend = () => {
      if (activeRef.current) setTimeout(startSession, 200);
      else setListening(false);
    };

    try { r.start(); } catch {}
  }, [lang, armSilence, onError]);
...
// ─────────────────────────────────────────────────────────────────────────────
// Answer scoring — runs locally, instant feedback
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// Tech detection + dynamic question banks
// ─────────────────────────────────────────────────────────────────────────────
const TECH_KW = {
  ...
  cloud:     ['aws','azure','gcp','docker','kubernetes','ci/cd','devops','terraform','microservice'],
  ml:        ['machine learning','deep learning','neural','nlp','model training','classification'],
  sysdesign: ['system design','scalab','load balanc','cache','cdn','kafka','message queue'],
};
...
const DRILLS = {
  react:    ["Walk me through how React's reconciliation algorithm decides what to re-render.","Explain useCallback vs useMemo with a real scenario where each prevents a performance issue.","How do you prevent unnecessary re-renders? Give 3 techniques with trade-offs."],
  java:     ["How does the JVM garbage collector work? Difference between young and old generation?","What is the difference between synchronized, volatile, and AtomicInteger? When do you use each?","Explain @Component vs @Service vs @Repository in Spring — does it matter which one you use?"],
  python:   ["Explain Python's GIL. How do you achieve real parallelism despite it?","Difference between a generator and list comprehension? When would you prefer one?","How does Python memory management work? Explain reference counting and cyclic GC."],
  nodejs:   ["How does Node.js handle thousands of concurrent connections? Explain the event loop phases.","What's the difference between process.nextTick(), setImmediate(), and setTimeout(0)?","How do you handle CPU-intensive tasks in Node.js without blocking the event loop?"],
  database: ["Explain clustered vs non-clustered index. When would adding an index hurt performance?","What is the N+1 query problem? How do you detect and fix it?","Explain ACID properties. How does a database ensure atomicity during a crash?"],
  cloud:    ["How do you handle zero-downtime deployments in Kubernetes?","Horizontal vs vertical scaling — when does horizontal break down?","How do you manage secrets in a containerized production application?"],
  ml:       ["How do you handle class imbalance? Trade-offs between oversampling and undersampling?","Explain bias-variance tradeoff. How do you diagnose which is causing underperformance?","What's the difference between L1 and L2 regularization and how do they affect your model?"],
  sysdesign:["Design a URL shortener like bit.ly for 100 million daily requests. Walk me through it.","How would you design a notification system delivering 1M push notifications per minute?","Explain consistent hashing. Why is it used in distributed cache clusters?"],
};
...
const HR_QS = [
  ...
  "Give an example of when you took initiative on something outside your responsibilities.",
  "Tell me about your biggest professional failure. What did you learn and what would you do differently?",
  "Describe a time you had to work under extreme pressure with a tight deadline.",
  "How do you handle critical feedback you disagree with? Give a real example.",
];

const GENERIC_Q = [
  "Walk me through the most challenging technical problem you've solved and how you debugged it.",
  ...
  "Tell me about a time a production issue occurred. What was your debugging approach?",
];
...
async function getDynamicNext({ msgs, answer, qNum, totalQ, role, type }) {
  ...
  const techCtx = techs.length ? `\nTechnologies detected in conversation: ${techs.join(', ')}` : '';

  const prompt = `You are an expert ${type||'technical'} interviewer at a top Indian tech company for the role: ${role}.${techCtx}
...
2. ${isLast
    ? 'Congratulate them warmly and give a 2-sentence overall performance summary.'
    : `Generate ONE sharp follow-up question that DIRECTLY builds on what they just said:
    - If they mentioned a specific tech → probe deeper into THAT technology specifically
    - If answer was vague (under 30 words, no examples) → ask for a concrete project example
    - If answer was strong → push with an edge case or failure scenario
    - For HR type → follow behavioral STAR format
...
{"feedback":"...","nextQuestion":${isLast?'null':'"..."'},"confidence":7,"keyMissing":"one missing thing or empty string"}`;
  ...
  try {
    const res = await fetch(`${API}/skillpath/dynamic-interview`, {
      method:'POST', headers:{...tk(),'Content-Type':'application/json'},
      body:JSON.stringify({ prompt, targetRole:role, interviewType:type, lastAnswer:answer, isLast }),
    });
    ...
    return localFallback(answer, techs, qNum, type, isLast);
  }
}
...
function localFallback(answer, techs, qNum, type, isLast) {
  ...
  if (isLast) return { feedback:'Good effort overall! Focus on adding concrete project examples and using the STAR format for even stronger answers.', nextQuestion:null, confidence:7, keyMissing:'' };

  for (const tech of techs) {
    if (DRILLS[tech]) return { feedback:`Good ${tech} knowledge.${!hasEg?' Tie your answer to a specific project next time.':''}`, nextQuestion:DRILLS[tech][qNum % DRILLS[tech].length], confidence:7, keyMissing:hasEg?'':'Specific project reference' };
  }

  if (isWeak) return { feedback:'Your answer was brief and lacked specifics. Use the STAR format and mention a real project you worked on.', nextQuestion:'Can you give me a specific example from one of your projects that illustrates what you just described?', confidence:4, keyMissing:'STAR example' };
  ...
  return { feedback:`${words>=60?'Good depth.':'Try to elaborate more.'} ${hasEg?'':'Mention a real project to make it memorable.'}`, nextQuestion:GENERIC_Q[qNum%GENERIC_Q.length], confidence:6, keyMissing:hasEg?'':'Project example' };
}
...
// ─────────────────────────────────────────────────────────────────────────────
// AI Avatar — animated SVG face with blink, lip-sync, gaze shift, breathing
// ─────────────────────────────────────────────────────────────────────────────
function AIAvatar({ isSpeaking, isThinking, isListening, persona, size=128 }) {
  ...
  useEffect(() => {
    let t;
    const blink = () => { setBlinkOpen(false); setTimeout(()=>setBlinkOpen(true),130); t=setTimeout(blink, 3000+Math.random()*2500); };
  ...
  useEffect(() => {
    ...
    let a;
    const tick = () => { setMouthOpen(0.25+Math.random()*0.75); a=setTimeout(tick,75+Math.random()*110); };
    tick(); return () => clearTimeout(a);
  }, [isSpeaking]);

  // Thinking brows
  useEffect(() => {
    if (!isThinking) { setBrowRaise(0); return; }
    const t = setInterval(()=>setBrowRaise(v=>v===0?3:0), 600);
  ...
  const eyeRY = blinkOpen ? 8 : 1;
  const mCY   = 70 + mouthOpen * 9;
  ...
  return (
    <div style={{ position:'relative', width:size, height:size, flexShrink:0 }}>
      ...
      <div style={{
        ...
        background: isSpeaking ? `conic-gradient(from 0deg,${col},#13a1a5,#47d372,${col})`
          : isListening ? 'conic-gradient(from 0deg,#13a1a5,#47d372,#13a1a5)'
          : isThinking  ? 'conic-gradient(from 0deg,#f59e0b,#ef4444,#f59e0b)'
          : `conic-gradient(from 0deg,${col},rgba(255,255,255,0.08),${col})`,
        animation: isSpeaking||isListening||isThinking ? 'avSpin 1.8s linear infinite' : 'avPulse 3.5s ease-in-out infinite',
        opacity:0.88,
      }}/>
      {/* Dark face bg */}
      <div style={{ position:'absolute', inset:7, borderRadius:'50%', background:'linear-gradient(145deg,#1c2b42,#0f1a2e)', border:`2px solid ${col}44`, overflow:'hidden', display:'flex', alignItems:'center', justifyContent:'center' }}>
        <svg width={size-14} height={size-14} viewBox="0 0 110 110" style={{animation:'breathe 3.8s ease-in-out infinite'}}>
          ...
          {blinkOpen && <>
            <ellipse cx={38+gaze.x} cy={53+gaze.y} rx="4.8" ry="5.5" fill="#16103a"/>
            <ellipse cx={72+gaze.x} cy={53+gaze.y} rx="4.8" ry="5.5" fill="#16103a"/>
            <ellipse cx={39.5+gaze.x} cy={51.2+gaze.y} rx="1.6" ry="1.8" fill="white" opacity="0.85"/>
            <ellipse cx={73.5+gaze.x} cy={51.2+gaze.y} rx="1.6" ry="1.8" fill="white" opacity="0.85"/>
          </>}
        ...
        {isThinking && (
          <div style={{position:'absolute',bottom:8,display:'flex',gap:5}}>
            {[0,1,2].map(i=><div key={i} style={{width:7,height:7,borderRadius:'50%',background:'#f59e0b',animation:`thinkB 1.2s ease-in-out ${i*0.2}s infinite`}}/>)}
          </div>
        )}
      </div>
      {/* Sound rings */}
      {isSpeaking && [1,2,3].map(i=><div key={i} style={{position:'absolute',inset:-(i*12),borderRadius:'50%',border:`1.5px solid ${col}44`,animation:`sndRing 1.6s ease-out ${i*0.32}s infinite`,pointerEvents:'none'}}/>)}
      {isListening && [1,2].map(i=><div key={i} style={{position:'absolute',inset:-(i*11),borderRadius:'50%',border:'1.5px solid rgba(19,161,165,0.5)',animation:`sndRing 1.3s ease-out ${i*0.3}s infinite`,pointerEvents:'none'}}/>)}
    </div>
  );
}
...
function ScorePanel({ m }) {
  ...
  return (
    <div style={{padding:'10px 16px',borderRadius:11,background:`${col}0d`,border:`1px solid ${col}20`,marginTop:10}}>
      ...
      {!m.hasEg && <div style={{marginTop:5,fontSize:'.7rem',color:'#92400e'}}>💡 Mention a project or experience to score higher</div>}
...
// ─────────────────────────────────────────────────────────────────────────────
// Mock Interview — the core component
// ─────────────────────────────────────────────────────────────────────────────
function MockInterview({ targetRole, interviewType, userName, onEnd }) {
  ...
  const speak = useCallback((text) => {
    ...
    utt.rate = 0.93; utt.pitch = 1.04; utt.volume = 1; utt.lang = 'en-IN';
    const getVoice = () => {
      const vv = window.speechSynthesis.getVoices();
      return vv.find(v=>v.name.includes('Google')&&v.lang==='en-IN') || vv.find(v=>v.lang.startsWith('en')&&!v.localService) || vv.find(v=>v.lang.startsWith('en')) || vv[0];
    };
    utt.onstart = () => setAiSpeaking(true);
    utt.onend   = () => setAiSpeaking(false);
    utt.onerror = () => setAiSpeaking(false);
    if (!window.speechSynthesis.getVoices().length) {
      window.speechSynthesis.onvoiceschanged = () => { window.speechSynthesis.onvoiceschanged=null; utt.voice=getVoice(); window.speechSynthesis.speak(utt); };
    } else { utt.voice=getVoice(); window.speechSynthesis.speak(utt); }
  }, []);
  ...
  useEffect(() => {
    const openQ = interviewType === 'HR'
      ? "Tell me about yourself — your educational background, internships or work experience, and what motivated you to pursue this career path."
    ...
    setTimeout(() => {
      const greeting = `Hello ${userName?.split(' ')[0]||'there'}! I'm ${persona.name}, ${persona.title} at ${persona.company}.\n\nI'll be conducting your ${interviewType} interview today for the ${targetRole} role.\n\n🎙️ Tip: Click the mic once → speak naturally → auto-sends after 2.5 seconds of silence. Or just type below.\n\n❓ Question 1/${totalQ}:\n\n${openQ}`;
  ...
  const sendAnswer = useCallback(async (textOverride) => {
    ...
    try {
      ...
      if (result.keyMissing) reply += `\n\n💡 Consider mentioning: ${result.keyMissing}`;
      if (result.nextQuestion) {
        ...
        reply += `\n\n🎉 Interview complete! Average score: ${avg}/100. Well done — keep practicing!`;
        setDone(true);
  ...
  return (
    <div style={{fontFamily:"'Nunito',sans-serif",background:'#fff',borderRadius:16,overflow:'hidden',border:'1px solid #e8edf5',boxShadow:'0 6px 28px rgba(4,44,93,0.1)'}}>
      ...
      {/* Header with avatar */}
      <div style={{background:'linear-gradient(135deg,#042c5d 0%,#1a0d3e 45%,#0c3240 100%)',padding:'20px 24px',display:'flex',alignItems:'center',gap:20}}>
        <AIAvatar isSpeaking={aiSpeaking} isThinking={loading} isListening={listening} persona={persona} size={128}/>
        <div style={{flex:1,minWidth:0}}>
          <div style={{fontFamily:"'Syne',sans-serif",fontWeight:900,fontSize:'1.1rem',color:'#fff'}}>{persona.name}</div>
        ...
        <div style={{flexShrink:0,textAlign:'right'}}>
          ...
          <div style={{fontSize:'.6rem',color:'rgba(255,255,255,0.4)',marginTop:1}}>Avg Score</div>
          <div style={{marginTop:6,fontSize:'.65rem',color:'rgba(255,255,255,0.45)',fontWeight:700}}>Q {Math.min(qNum+1,totalQ)}/{totalQ}</div>
        </div>
      </div>
      ...
      {/* Messages */}
      <div style={{minHeight:340,maxHeight:440,overflowY:'auto',padding:'18px 20px 8px',background:'#f8f9fc',display:'flex',flexDirection:'column'}}>
        {msgs.map((m,i)=>(
          <div key={i} style={{display:'flex',justifyContent:m.role==='user'?'flex-end':'flex-start',marginBottom:12}}>
            {m.role==='ai' && <div style={{width:32,height:32,borderRadius:'50%',background:`linear-gradient(135deg,#042c5d,${persona.color})`,display:'flex',alignItems:'center',justifyContent:'center',fontFamily:"'Syne',sans-serif",fontWeight:900,fontSize:'.8rem',color:'#fff',flexShrink:0,marginRight:10,alignSelf:'flex-end'}}>{persona.name[0]}</div>}
            <div style={{maxWidth:'78%',padding:'12px 16px',borderRadius:m.role==='user'?'18px 18px 4px 18px':'18px 18px 18px 4px',background:m.role==='user'?'linear-gradient(135deg,#531697,#13a1a5)':'#fff',color:m.role==='user'?'#fff':'#0f1a2e',border:m.role==='user'?'none':'1px solid #e8edf5',boxShadow:m.role==='user'?'0 4px 14px rgba(83,22,151,0.22)':'0 2px 10px rgba(4,44,93,0.05)',fontSize:'.875rem',lineHeight:1.7,whiteSpace:'pre-wrap',fontFamily:"'Nunito',sans-serif"}}>
              {m.loading?<span style={{opacity:.4,animation:'blink .8s infinite'}}>▋</span>:m.content}
            </div>
            {m.role==='user' && <div style={{width:32,height:32,borderRadius:'50%',background:'#e8edf5',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'.85rem',flexShrink:0,marginLeft:10,alignSelf:'flex-end'}}>👤</div>}
          </div>
        ))}
        {listening && liveText && (
      ...
      <div style={{padding:'14px 20px',borderTop:'1px solid #e8edf5',background:'#fff'}}>
        ...
        <div style={{marginTop:8,display:'flex',gap:16,fontSize:'.68rem',color:'#b0bec9',flexWrap:'wrap'}}>
          ...
          <span>⌨️ Enter to send · Shift+Enter for newline</span>
            ...
            <div style={{display:'flex',gap:4,alignItems:'flex-end'}}>
              {scores.map((s,i)=>(
                <div key={i} style={{display:'flex',flexDirection:'column',alignItems:'center',gap:2}}>
                  <div style={{width:26,borderRadius:'3px 3px 0 0',height:`${Math.max(4,s*0.38)}px`,background:s>=75?'#47d372':s>=50?'#f59e0b':'#ef4444',transition:'height .5s ease'}}/>
...
export default function InterviewPrepPage() {
  ...
  async function runDeep() {
    ...
    setDeepLoading(true);setDeepResult(null);
  ...
  return (
    <div style={{fontFamily:"'Nunito',sans-serif",maxWidth:940,margin:'0 auto'}}>
      ...
      <div style={{marginBottom:22}}>
        <h1 style={{fontFamily:"'Syne',sans-serif",fontWeight:900,fontSize:'1.65rem',color:'#0f1a2e',display:'flex',alignItems:'center',gap:10,flexWrap:'wrap',margin:0}}>
          🎤 AI Interview Coach
          <span style={{padding:'4px 12px',borderRadius:999,background:'linear-gradient(135deg,rgba(83,22,151,0.1),rgba(19,161,165,0.1))',color:'#531697',fontSize:'.7rem',fontWeight:800,border:'1px solid rgba(83,22,151,0.15)'}}>Human AI · Adaptive · Voice Enabled</span>
        </h1>
        <p style={{color:'#7a8ba8',marginTop:6,fontSize:'.88rem',marginBottom:0}}>Your AI interviewer adapts every question to your previous answer — just like a real interview panel.</p>
      </div>
              ...
              <div key={q._id} style={{padding:'14px 18px',marginBottom:10,background:'#fff',border:'1px solid #e8edf5',borderRadius:13}}>
                ...
                {isOpen && (
                  <div style={{marginTop:14,paddingTop:14,borderTop:'1px solid #f0f3fa'}}>
                    ...
                    <button onClick={()=>getAiAns(q._id,q.question)} disabled={aiAnsLoad[q._id]} style={{padding:'7px 16px',borderRadius:8,border:'none',background:aiAnsLoad[q._id]?'#e8edf5':'linear-gradient(135deg,#531697,#13a1a5)',color:aiAnsLoad[q._id]?'#b0bec9':'#fff',fontWeight:700,cursor:'pointer',fontSize:'.78rem',fontFamily:"'Nunito',sans-serif"}}>
                      {aiAnsLoad[q._id]?'⏳ Loading…':'🤖 Get AI Answer'}
                    </button>
                  </div>
                )}
              </div>
          ...
          {!mode && (
            <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(260px,1fr))',gap:14}}>
              ...
              <div onClick={()=>setMode('mock')}
                ...
                <div style={{fontSize:'.8rem',color:'rgba(255,255,255,0.6)',lineHeight:1.6,marginBottom:14}}>Human-like AI adapts every question to your answer. Voice enabled — speak naturally.</div>
              ...
              <div onClick={()=>{setMode('prep');runPrep();}}
                ...
                <div style={{fontSize:'.8rem',color:'#7a8ba8',lineHeight:1.6}}>Personalised: technical questions, behavioral questions, skill gaps, quick wins.</div>
              </div>
            ...
            <div>
              <div style={{background:'#fff',border:'1px solid #e8edf5',borderRadius:14,padding:'20px 22px',marginBottom:16}}>
                ...
                <div style={{display:'flex',gap:10}}>
                  ...
                  <button onClick={runDeep} disabled={!deepTopic.trim()||deepLoading} style={{padding:'10px 24px',borderRadius:9,border:'none',background:!deepTopic.trim()||deepLoading?'#e8edf5':'linear-gradient(135deg,#531697,#13a1a5)',color:!deepTopic.trim()?'#b0bec9':'#fff',fontWeight:800,cursor:'pointer',fontFamily:"'Nunito',sans-serif"}}>
                    {deepLoading?'…':'Dive In →'}
                  </button>
                </div>
              </div>
